...
\item
  ...
  Run the command ``R'' in terminal. An R Console should start and you should be able to start running R commands.
  ...
  \begin{quote}
  Note: \textbf{We will not be using the R console}, instead, we will be using RStudio!
  \end{quote}
\item
...
\item
  ...
  \begin{quote}
  For example: ``File '\ldots\_svg-tex.pdf' is missing.'' is a bug that only occurs when producing bookdown::pdf\_book. It occurs since you are calling a svg image in regular markdown, which is not allowed. If you want to display a .svg image, use HTML code.
  \end{quote}
\end{itemize}
